...
class Common {
  ...
  Future getAllProducts() async {
    var prodAll = await wc_api.getAsync("products?page=1&per_page=15");
    return prodAll;
  }

  Future getSliderProducts() async {
    var prodSlider = await wc_api.getAsync("products?category=94");
    return prodSlider;
  }

  Future loadMoreProducts(page) async {
    var loadMore = await wc_api.getAsync("products?page=$page&per_page=15");
    return loadMore;
  }

  Future loadMoreProductsWithCategory(cid, page) async {
    var loadMoreWithCategory =
        await wc_api.getAsync("products?category=$cid&page=$page&per_page=15");
    return loadMoreWithCategory;
  }

  Future getProductDetails(pid) async {
    var productDetails = await wc_api.getAsync("products/$pid");
  ...
  Future getRelatedProduct(cid) async {
    ...
    for (var i = 0; i < prodLength; i++) {
      var relatedProducts = await wc_api.getAsync("products/${cid[i]}");
  ...
  Future getProductCategories() async {
    var prodCategories =
        await wc_api.getAsync("products/categories?parent=0&order=desc");
    return prodCategories;
  }

  Future getSubCategories(cid) async {
    var subCategories =
        await wc_api.getAsync("products/categories?parent=$cid&order=desc");
    return subCategories;
  }

  Future getCategoryProducts(cid) async {
    var catProducts = await wc_api.getAsync("products?category=$cid");
    return catProducts;
  }
}
